...
class GazpromSpider(scrapy.Spider):
    ...
    def vacancy_data(self, response):
        ...
        scraped_info['job_title'] = raw_data.xpath(
            '//*[@id="content-normal"]/article/h1/text()').extract()

        # data for company_name info
        scraped_info['company_name'] = response.xpath(
            '//*[@id="content-normal"]/article/dl/dd[1]/a/text()').extract()
    ...
    def parse(self, response):
        ...
        page_link = response.xpath('//li[@class="current"]/following::li[1]/a/@href')

        if page_link:
            print('next page = ', page_link.get())
            yield  scrapy.Request(url='https://www.gazpromvacancy.ru/'+ page_link.get(), callback= self.parse)
